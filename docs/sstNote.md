...
□ 严格遵守：
	...
	e. Mysql 在 D:\wamp64\bin\mysql\mysql8.0.31
	   127.0.0.1 DB 的 acc/pwd： root / 
	   (no password)
...
以下是用来重算 KD 和布林带宽的程序脚本
从 2025 年 1 月 1 号开始计算到最后一个交易日，就是最近
不过在执行这个 code 之前，请确认 

# 打开部署文档
notepad DEPLOYMENT-KD-BOLLINGER-BATCH.md
...
d:\wamp64\bin\mysql\mysql8.0.31\bin\mysql.exe -uroot -sN -e "USE sst; SELECT COUNT(DISTINCT StockDate) FROM stock60days WHERE StockDate >= '2025-01-01' AND (KD_K IS NULL OR KD_K = 0);"