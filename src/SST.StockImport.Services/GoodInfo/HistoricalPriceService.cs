...
/// <summary>
/// 歷史股價處理服務（股價創五年高點）
/// 對應舊系統：appCommon.cs 的 歷史股價_Insert 方法 (lines 6093-6185)
/// linkLabel23: 股價創五年高點
/// </summary>
public class HistoricalPriceService
{
    ...
    public HistoricalPriceService(ITradeDataRepository tradeDataRepository)
    {
    ...
    /// <param name="recNote">記錄註記（例如："股價創五年高點"）</param>
    /// <returns>更新的記錄數量</returns>
    public async Task<int> ProcessHistoricalPriceCsvAsync(string csvFilePath, string recNote = "股價創五年高點")
    {
        ...
        const int dateIdx = 7; // 歷史股價的日期在第7欄

        foreach (var line in dataLines)
        {
            var columns = ParseCsvLine(line);
            if (columns.Length < 8) continue; // 確保至少有第7欄（索引7）
